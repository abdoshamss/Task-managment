...
class _CutomAutoCompleteTextFeildState<T>
  ...
  PagingController<int, T> pagingController = PagingController<int, T>(
    firstPageKey: 1,
  );
  void addPageListers() {
  ...
  Future<void> getItems(int pageKey) async {
      ...
      child: Column(
        ...
        children: [
          if (widget.showLabel) ...[
            Row(
              children: [
                CustomText(widget.lable.toString()),
                const SizedBox(width: 5),
                if (widget.showRequiredStar) CustomText("*", color: Colors.red),
              ],
            ),
  ...
  Future<void> updateSuggestions(String input, {bool refresh = false}) async {
    ...
    _debounce = Timer(
      Duration(milliseconds: widget.localData ? 0 : 600),
      () async {
